...
print ("This is a dynamic plot that can be zoomed in on, and if you hover over a data point, you can see what grain it is")
...
Evans_hefty<- ggplot () + 
  ...
  geom_errorbar(data = ME_hefty_flatlist, aes(x=eU, ymin = CorrDate_mean - CorrDate_SD, ymax = CorrDate_mean + CorrDate_SD), color = 'black', width=.1) +
...
### Hefty Outputs
Here, I've chosen to exclude the grain from LP 1 with a date of 137 ma, beucase it is an outlier from the rest of the data 
###### Option #1 
This grouping takes into account the 'populations' approach, it groups grains based on eU into what visually look like clusters
**4 Bins:**  
*0 - 500 (4 grains)  
*500 - 900 (2 grains)  
*900 - 1600 (7 grains)  
...
###### Option #2: 
This grouping is tempting becuase it takes the first three bins of Pikes Peak, collapses them into 1, and then uses the same boundaries as bins 4 and 5 
**3 Bins:**
*0 - 500 (4 grains)
*500 - 9000 (2 grains)  
*900 - 2000 (9 grains)
...
print(LP_hefty_flatlist)
write.csv(LP_hefty_flatlist, file="data_output/Evans Hefty Flatlist.csv")
```
...
```{r, echo = FALSE, warning= FALSE}
Longs_hefty<- ggplot () + 
  geom_errorbar(data = LP_hefty_flatlist, aes(x=eU, ymin = CorrDate_mean - CorrDate_SD, ymax = CorrDate_mean + CorrDate_SD), color = 'black', width=.1)+
  geom_point(data=Longs_Peak %>% filter (!Donotuse), aes(x=eU, y=Corr_Date, label = papername, text = Grain), fill = 'gray60', pch=22, size=4) + 
  geom_point(data = LP_hefty_flatlist, aes(x = eU, y = CorrDate_mean), pch = 21, size = 6, fill = 'darkgreen') +
  geom_vline(xintercept = 500, linetype= "dashed")  + 
  geom_vline(xintercept = 1000, linetype = "dashed") +  
  geom_vline(xintercept = 1500,linetype="dashed")  + 
  labs(x="eU (ppm)", y = "Date (Ma)") 
...
print ("This is a dynamic plot that can be zoomed in on, and if you hover over a data point, you can see what grain it is")
ggplotly(Longs_hefty, dynamicTicks = TRUE)  
print(Longs_hefty)
