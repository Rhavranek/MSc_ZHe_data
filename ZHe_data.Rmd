...
knitr::opts_chunk$set(
  ...
  dev.args = list(
    pdf = list(encoding = "WinAnsi", useDingbats = FALSE)
    ),
  fig.path = file.path(
    "fig_output", paste0(gsub("\\.[Rr]md", "", knitr::current_input()), "_")
    )
)
```
...
In this chunk I modify the sample names, for ease of reading, where PP still stands for Pikes Peak, and they are numbered 1 - 6 with the lowest elevation sample being 1 and highest elevation sample being 6. To do that I use nested if/else statements. In the first line I, I ask is the sample PP2084?, if true, add in information to the column named papername and call that sample PP1, else move onto the next sample. That goes down the list until the last until PP5, where the only sample left is PP6. 
```{r}
pikes_data <- read.csv("data/Pikes_Data_for_R .csv")
pikes_data <- pikes_data %>%
    mutate(papername =
              ifelse(Sample == "PP2084", "PP1", #If True, add label PP1
                ifelse(Sample == "PP2479", "PP2",
                  ifelse(Sample == "PP2907", "PP3",
                    ifelse(Sample == "PP3597", "PP4",
                      ifelse(Sample == "PP3971", "PP5", "PP6")
                    )
                  )
                )
              )
...
pp_ed_by_eu <- ggplot(data = pikes_data, aes(Corr_Date, Elevation)) +
  geom_errorbarh(aes(
    xmin = Corr_Date - Corr_date_unc,
    xmax = Corr_Date + Corr_date_unc),
    color = "black",
    height = 0) +
  geom_point(aes(fill = eU),
    shape = 22, size = 4) +
  viridis::scale_fill_viridis(option = "viridis", limits = c(0, 2000)) +
  geom_point(
    data = aft_data %>% filter(Peak == "Pikes Peak"),
    aes(aft_date, Elevation), pch = 15, size = 4) +
  theme_classic() +
  ...
  guides(linetype = guide_legend(
    label.hjust = 0, label.vjust = 0.5, keyheight = 2)
    )
...
pp_deu <- ggplot(data = pikes_data, aes(x = eU, y = Corr_Date)) +
  geom_errorbar(aes(
    ymin = Corr_Date - Corr_date_unc,
    ymax = Corr_Date + Corr_date_unc),
    color = "black", width = .1) +
  geom_errorbarh(aes(
    xmin = eU - eU_unc,
    xmax = eU + eU_unc)) +
...
pp_grainsize_elev <- ggplot(data = pikes_data, aes(r, Corr_Date)) +
  ...
  labs(
    x = "Grain Size",
    y = "Date",
    title = "Date vs. Grain Size, colored by elevation") +
  theme(
...
pp_grainsize_eu <- ggplot(data = pikes_data, aes(r, eU)) +
  ...
  labs(
    x = "Grain size (µm)",
    y = "eU",
    title = "eU vs. Grain Size, colored by elevation") +
  theme(
    panel.border = element_rect(fill = NA, size = 1),
    text = element_text(size = 12),
    legend.position = "none",
...
```{r}
grains_not_modeled <- c(6, 30)

pikes_data <- pikes_data %>%
  mutate( 
    Rownumber = row_number(),
    Donotuse = (Rownumber %in% grains_not_modeled),
    bindata = cut(eU, c(0, 250, 600, 1000, 2500))) #these are my bin cutoff
...
```{r, echo = FALSE}
pikes_lowelevation_hefty <- pikes_data %>%
  filter(papername == "PP1" | papername == "PP2" | papername == "PP3") %>%
    filter(!Donotuse) %>%
      group_by(bindata) %>%
    summarize(
...
pikes_highelevation_hefty <- pikes_data %>%
  filter(papername != "PP1" & papername != "PP2" & papername != "PP3") %>%
    filter(!Donotuse) %>%
      group_by(bindata) %>%
    summarize(
...
#For Reveiwer #2's comments we wanted to demonstrate all of the pikes peak data modelled together:
pikes_all_hefty <-  pikes_data %>%
  filter(!Donotuse) %>%
  group_by(bindata) %>%
    summarize(
      U = round(mean(U), digits = 1),
...
print(pikes_all_hefty)
#write.csv(Pikes_all_hefty, file="data_output/Pikes ALL Elevations Hefty Flatlist.csv")
```
...
Pikes_lowelevation_heftyplt<- ggplot () + 
  geom_point(data=pikes_data %>% filter (papername == "PP1" | papername == "PP2" | papername == "PP3") %>% filter (!Donotuse) , aes(x=eU, y=Corr_Date, label = papername, text = Grain),  fill = 'gray60', pch=22, size=4) + 
  ...
  geom_point(data = Pikes_lowelevation_hefty, aes(x = eU, y = CorrDate_mean), pch = 25, size = 6, fill = 'dodgerblue')+
  geom_point(data = model_outcomes %>% filter(Peak == "Pikes") %>% filter(data_subset == "LOW"), aes(x = eU, y=model_date), pch =21, size = 6, fill = 'magenta' ) +
  geom_vline(xintercept = 250, linetype = "dashed") +  
  geom_vline(xintercept = 500,linetype="dashed")  + 
  geom_vline(xintercept = 1000,linetype="dashed")  + 
  labs(x="eU (ppm)", y = "ZHe Date (Ma)", title = "Pikes Peak Low Elevation Samples (< 3500 m)" ) +  
  scale_x_continuous(expand =c(0,0), limits=c(0,3200), breaks=seq(0, 3200, 800)) + 
...
Pikes_highelevation_heftyplt<- ggplot () + 
  ...
  geom_point(data = Pikes_highelevation_hefty, aes(x = eU, y = CorrDate_mean), pch = 25, size = 6, fill = 'dodgerblue') +
  geom_point(data = model_outcomes %>% filter(Peak == "Pikes") %>% filter(data_subset == "HIGH"), aes(x = eU, y=model_date), pch =21, size = 6, fill = 'magenta' ) +
  geom_vline(xintercept = 250, linetype = "dashed") +  
...
#### Where is the misfit in the high elevation model?
As a part of the revision process we were having trouble getting fits on the high elevation data set so here I plotted the outcomes of the low elevation model (where we got lots of fits) onto the high elevation dataset. From this we decided its probably the rigidity of the 3rd and 4th bins. So, we boosted the uncertainty on those bins. 
```{r}
low_outcomes_210909 <- tibble(model_date = 651.7, eU = 144.5) %>% 
  add_row (model_date = 424.7, eU = 422.9) %>% 
  add_row (model_date = 165.1, eU = 749.7) %>% 
  add_row (model_date = 70.3, eU = 1622.1)


Pikes_highelevation_misfit<- ggplot () + 
  ...
  geom_errorbar(data = Pikes_highelevation_hefty, aes(x=eU, ymin = CorrDate_mean - model_uncertainty, ymax = CorrDate_mean + model_uncertainty), color = 'black', width=.1) +
  geom_point(data = Pikes_highelevation_hefty, aes(x = eU, y = CorrDate_mean), pch = 25, size = 6, fill = 'dodgerblue') +
  geom_point(data = low_outcomes_210909, aes(x = eU, y=model_date), pch =21, size = 6, fill = 'magenta' ) +
  geom_vline(xintercept = 250, linetype = "dashed") +  
  geom_vline(xintercept = 600,linetype="dashed")  + 
  geom_vline(xintercept = 1000,linetype="dashed")  + 
  labs(x="eU (ppm)", y = "ZHe Date (Ma)",  title = "Pikes Peak High Elevation Samples (>3500 m)" ) +  
  scale_x_continuous(expand =c(0,0), limits=c(0,3200), breaks=seq(0, 3200, 400)) + 
  scale_y_continuous(expand =c(0,0), limits = c(0,800), breaks=seq(0, 800, 200)) +
  theme_classic()+
  theme(
   panel.border = element_rect(fill=NA, colour = "black", size = 1),
...
print(Pikes_highelevation_misfit)
```
...
ME_reU<-ggplot (data=Mount_Evans, aes(r, eU)) + 
  ...
  viridis::scale_fill_viridis(option = "magma", limits = c(2000,4500))+
  labs(x="Grain Size (µm)", y = "eU (ppm)", title = "Grain size vs. eU" ) +  
  scale_y_continuous(expand=c(0,0), limits=c(0,3200), breaks=seq(0, 3200, 400)) + 
  scale_x_continuous(expand=c(0,0), limits = c(0,140), breaks=seq(0, 140, 30)) +
  theme_classic() +
  theme(
    panel.border=element_rect(fill=NA,size = 1),
...
ME1_Hefty<- ggplot () + 
  ...
  geom_point(data = ME1_flatlist, aes(x = eU, y = CorrDate_mean), pch = 25, size = 6, fill = 'dodgerblue') +
  geom_point(data = model_outcomes %>% filter(Peak == "Evans") %>% filter(data_subset == "LOW"), aes(x = eU, y=model_date), pch =21, size = 6, fill = 'magenta' ) +
  #geom_vline(xintercept = 250, linetype= "dashed")  + 
...
Evans_highelevation_hefty<- ggplot () + 
  ...
  geom_point(data = ME_highelevation_flatlist, aes(x = eU, y = CorrDate_mean), pch = 25, size = 6, fill = 'dodgerblue') +
  geom_point(data = model_outcomes %>% filter(Peak == "Evans") %>% filter(data_subset == "HIGH"), aes(x = eU, y=model_date), pch =21, size = 6, fill = 'magenta' ) +
  geom_vline(xintercept = 450, linetype = "dashed") +  
  geom_vline(xintercept = 1000,linetype="dashed")  + 
...
LP_highelevation_flatlist <- Longs_Peak %>% filter (Elevation > 3500) %>% 
  ...
  group_by(bindata) %>% 
    summarize(
      U = round (mean(U), digits = 1),
      Th = round (mean(Th), digits = 1),
      RawDate_mean =  round (mean(Uncorr_date), digits = 2),
      Rawdate_15percent = round (0.15*mean(Uncorr_date), digits = 2),
      Rawdate_SD = round (sd(Uncorr_date), digits = 2),
...
```{r, echo = FALSE}
LP_allelevations_flatlist <- Longs_Peak %>% 
  filter (!Donotuse) %>% 
  group_by(bindata) %>% 
    summarize(
      U = round (mean(U), digits = 1),
...
print(LP_allelevation_flatlist)
write.csv(LP_allelevations_flatlist, file="data_output/Longs all Elevation Hefty Flatlist.csv")
```
...
```{r LP highelevation hefty plot, echo = FALSE, warning= FALSE}
Longs_highelevation_hefty<- ggplot () + 
  ...
  geom_point(data=Longs_Peak %>% filter (!Donotuse) %>% filter (Elevation >3500), aes(x=eU, y=Corr_Date, label = Sample, text = Grain), fill = 'gray60', pch=22, size=4) + 
  geom_point(data = LP_highelevation_flatlist, aes(x = eU, y = CorrDate_mean), pch = 25, size = 6, fill = 'dodgerblue') +
  geom_point(data = model_outcomes %>% filter(Peak == "Longs") %>% filter(data_subset == "HIGH"), aes(x = eU, y=model_date), pch =21, size = 6, fill = 'magenta' ) +
  geom_vline(xintercept = 500, linetype= "dashed")  + 
  geom_vline(xintercept = 1000, linetype = "dashed") +  
...
Longs_lowelevation_hefty<- ggplot () + 
  ...
  geom_point(data=Longs_Peak %>% filter (!Donotuse) %>% filter (Elevation  <= 3500), aes(x=eU, y=Corr_Date, label = Sample, text = Grain), fill = 'gray60', pch=22, size=4) + 
  geom_point(data = LP_lowelevation_flatlist, aes(x = eU, y = CorrDate_mean), pch = 25, size = 6, fill = 'dodgerblue') +
  geom_point(data = model_outcomes %>% filter(Peak == "Longs") %>% filter(data_subset == "LOW"), aes(x = eU, y=model_date), pch =21, size = 6, fill = 'magenta' ) +
  geom_vline(xintercept = 500, linetype= "dashed")  + 
  geom_vline(xintercept = 1000, linetype = "dashed") +  
  labs(x="eU (ppm)", y = "Date (Ma)", title= "Longs Peak, elevation <= 3500 m") +
  scale_x_continuous(expand =c(0,0), limits=c(0,3200), breaks=seq(0, 3200, 800)) + 
