...
Pikes_highelevation_heftyplt<- ggplot () + 
  ...
  geom_point(data = model_outcomes %>% filter(Peak == "Pikes") %>% filter(data_subset == "HIGH"), aes(x = eU, y=model_age), pch =25, size = 6, fill = 'magenta' ) +
  geom_vline(xintercept = 250, linetype = "dashed") +  
...
ME1_Hefty<- ggplot () + 
  ...
  geom_point(data = model_outcomes %>% filter(Peak == "Evans") %>% filter(data_subset == "LOW"), aes(x = eU, y=model_age), pch =25, size = 6, fill = 'magenta' ) +
  #geom_vline(xintercept = 250, linetype= "dashed")  + 
